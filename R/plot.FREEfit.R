...
plot.FREEfit <- function(x, ...) {
  ...
  old.par$page <- NULL  
  vals <- coef(x)
  if (all(sapply(vals, is.null))) {
    cat('Model was fitted without functional data; plot delta coefficients
         as you would for a standard linear model')
  } else {
    ...
    plot(c(x$fitted), c(x$observed), bty = 'l',
    ...
    plot(x$llik_all[[1]], type = 'l',
         ylim = c(min(unlist(x$llik_all)), max(unlist(x$llik_all))),
    ...
    for (i in 1:nrow(vals$mean)) {
      ...
      plot(vals$mean[i, ] ~ bins_tmp,
           type = 'l', bty = 'l',
           xlab = "Argument", ylab = y.lab[i],
           ylim = c(y.min, y.max), las = 1, ...)
      if (!is.null(vals$upper) & !is.null(vals$lower)) {
        lines(vals$upper[i, ] ~ bins_tmp, lty = 2)
        lines(vals$lower[i, ] ~ bins_tmp, lty = 2)
      }
      if ((i > 1) | (x$method == "scalar")) {
        lines(c(min(bins_tmp), max(bins_tmp)), c(0, 0), lty = 3)
      }
    }
    if (is.null(vals$upper) | is.null(vals$lower)) {
    ...
    if (!is.null(x$fp.sd.mean)) {
      ...
      sd.upper <- c(sqrt(max(x$sigma2.mean + 1.96 * x$sigma2.sd, 0)),
                    x$fp.sd.mean + 1.96 * x$fp.sd.sd)
      sd.upper <- ifelse(sd.upper < 0, 0, sd.upper)
      sd.lower <- c(sqrt(max(x$sigma2.mean - 1.96 * x$sigma2.sd, 0)),
                    x$fp.sd.mean - 1.96 * x$fp.sd.sd)
      sd.lower <- ifelse(sd.lower < 0, 0, sd.lower)
      par(mar=c(5.1, 7.1, 4.1, 2.1))
      plot(sd.mean[1], 1, pch = 16, cex = 2,
           xlim = c(0, max(sd.upper)),
           ylim = c(0, length(sd.mean) + 1),
           las = 1, bty = 'l',
           xlab = "Standard deviations", yaxt = "n", ylab = "")
      ...
      axis(side = 2, at=c(1:length(sd.mean)),
           labels = c("Residual", paste0("Group", 1:length(x$fp.sd.mean))),
           las = 1)
