                            ...
                            ...)
{
  if (is.matrix(y)) {
    ...
    if (!is.null(nrow(x))){
      if (nrow(y) != nrow(x)) {
        stop("response and predictor data should have one row for each site",
             call. = FALSE)
      }
    }
    if (!is.null(z)) {
      warning("matrix z is only used for scalar models",
              call. = FALSE)
    }
    if (is.null(groups)) {
      groups <- matrix(sample(c(1), size = nrow(y), replace = TRUE), ncol = 1)
      model <- FREEspline(y = y, x = x,
                          groups = groups,
                          bins = bins,
                          n.iters = n.iters,
                          n.burnin = n.burnin,
      ...
      if (is.matrix(groups) | is.integer(groups) | is.numeric(groups)) {
        if (is.integer(groups)) {
          ...
          if (is.numeric(groups)) {
            warning("a numeric vector has been provided for groups;
                    make sure that this vector is assigning each row
                    of the response to a cluster",
                    call. = FALSE)
            groups <- as.matrix(groups)
          }
        }
        if (nrow(groups) != nrow(y)) {
          stop("response and groups should have the same number of rows",
               call. = FALSE)
        }
        ...
        stop("the groups variable needs to be a matrix or integer/numeric vector",
             call. = FALSE)
      }
    }
  } else {
    if (is.numeric(y)) {
      ...
      if (is.matrix(groups) | is.integer(groups) | is.numeric(groups)) {
        ...
        if (is.numeric(groups)) {
          warning("a numeric vector has been provided for groups;
                  make sure that this vector is assigning each observation of
                  the response to a cluster",
                  call. = FALSE)
          groups <- as.matrix(groups)
        }
        if (nrow(groups) != length(y)) {
          stop("response and groups should have the same number of rows/observations",
               call. = FALSE)
        }
        groups <- apply(groups, 2, function(x) as.integer(as.factor(x)))
      }
      model <- FREEscalar(y = y,
                          x = x,
                          z = z,
                          groups = groups,
                          bins = bins,
      ...
      stop("model response must be a numeric vector (scalar response model) or a matrix (function response model)",
           call. = FALSE)
    }
  }
  class(model) <- "FREEfit"
