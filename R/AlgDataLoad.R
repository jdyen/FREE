AlgDataLoad <- function(data.subset=NULL, log.scale=TRUE){
  ...
  preds.fish[data.subset, c(1:7, 10:11)] <- apply(preds.fish[data.subset, c(1:7, 10:11)],
                                                  2,stdx)
  predictors <- as.matrix(preds.fish[data.subset, c(2, 3, 6, 7, 10, 11)])
  ...
  bin.data <- size.class
  coord.data <- coords
  colnames(X.data) <- c("CARBON", "PHOS", "NITRO", "LIGHT", "ELEV", "DRAIN")
