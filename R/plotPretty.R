plotPretty <- function(x, ...) {
  ...
  if (x$method == "scalar") {
    for (i in 1:nrow(vals$mean)) {
      plot(vals$mean[i, ] ~ x$bins[[i]],
           type = 'l', bty = 'l',
           xlab = "Argument",
           ylab = "Effect on response",
           ylim = c(min(vals$lower[i, ], vals$upper[i, ]),
                    max(vals$lower[i, ], vals$upper[i, ])),
           las = 1, cex.lab = 1.25, ...)
      polygon(c(x$bins[[i]], rev(x$bins[[i]])),
              c(vals$lower[i, ], rev(vals$upper[i, ])),
              col = "gray75",
              border = NA)
      lines(vals$mean[i, ] ~ x$bins[[i]])
      lines(c(min(x$bins[[i]]), max(x$bins[[i]])), c(0, 0), lty=2)
    }
  } else {
